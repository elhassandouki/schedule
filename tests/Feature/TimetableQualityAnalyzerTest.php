...
class TimetableQualityAnalyzerTest extends TestCase
{
    ...
    public function test_completely_valid_timetable_has_no_warnings(): void
    {
        ...
        TimetableSession::create(['subject_id' => $data['subject']->id, 'teacher_id' => $data['teacher']->id, 'classroom_id' => $data['classroom']->id, 'student_group_id' => $data['section']->id, 'semester_id' => $data['semester']->id, 'day_id' => $data['day']->id, 'timeslot_id' => $data['timeslot']->id]);
        TimetableSession::create(['subject_id' => $data['subject2']->id, 'teacher_id' => $data['teacher2']->id, 'classroom_id' => $data['classroom']->id, 'student_group_id' => $data['section2']->id, 'semester_id' => $data['semester']->id, 'day_id' => $data['day2']->id, 'timeslot_id' => $data['timeslot2']->id]);
    ...
    public function test_skipped_sessions_are_reflected_in_generation_report(): void
    {
        ...
        Subject::create(['teacher_id' => Teacher::create(['name' => 'Prof X'])->id, 'name' => 'Maths', 'code' => 'MTH-'.Str::random(4), 'sessions_per_week' => 1]);
    ...
    public function test_teacher_overload_and_group_overload_are_warnings(): void
    {
        ...
        for ($i = 0; $i < 7; $i++) {
            ...
            TimetableSession::create(['subject_id' => $data['subject']->id, 'teacher_id' => $teacher->id, 'classroom_id' => $data['classroom']->id, 'student_group_id' => $section->id, 'semester_id' => $data['semester']->id, 'day_id' => $day->id, 'timeslot_id' => $timeslot->id]);
        }
        $analysis = (new TimetableQualityAnalyzer())->analyze($data['semester']->id);
    ...
    public function test_excessive_gaps_and_consecutive_sessions_create_warnings(): void
    {
        $data = $this->seedSemesterWithSessions();
        TimetableSession::create(['subject_id' => $data['subject']->id, 'teacher_id' => $data['teacher']->id, 'classroom_id' => $data['classroom']->id, 'student_group_id' => $data['section']->id, 'semester_id' => $data['semester']->id, 'day_id' => $data['day']->id, 'timeslot_id' => $data['timeslot']->id]);
        TimetableSession::create(['subject_id' => $data['subject']->id, 'teacher_id' => $data['teacher']->id, 'classroom_id' => $data['classroom']->id, 'student_group_id' => $data['section']->id, 'semester_id' => $data['semester']->id, 'day_id' => $data['day']->id, 'timeslot_id' => $data['timeslot2']->id]);
        TimetableSession::create(['subject_id' => $data['subject']->id, 'teacher_id' => $data['teacher']->id, 'classroom_id' => $data['classroom']->id, 'student_group_id' => $data['section']->id, 'semester_id' => $data['semester']->id, 'day_id' => $data['day']->id, 'timeslot_id' => $data['timeslot3']->id]);
        $analysis = (new TimetableQualityAnalyzer())->analyze($data['semester']->id);
        $this->assertNotEmpty($analysis['soft_warnings']);
    }
